Attribute VB_Name = "generalreport"
...
Application.DisplayAlerts = False
    ...
    Planinformation = "Collect goods 12~14th December"
    ...
    For i = 0 To 70
            ...
            Else
                ...
                Else
                    ...
                    orderno(i) = ts.Value
                    orderqty(i) = Application.Sum(TWS1.Range("J" & orderrowstart(i) & ":J" & orderrowfinish(i) - 1))
                    orderamount(i) = Round(Application.Sum(TWS1.Range("L" & orderrowstart(i) & ":L" & orderrowfinish(i) - 1)), 2)
                    orderctn(i) = Application.Sum(TWS1.Range("I" & orderrowstart(i) & ":I" & orderrowfinish(i) - 1))
    

                    For j = modelstart(i) To modelfinish(i)
                    wks1.Range("v" & j).Value = wks1.Range("v" & j).Value
                    Next
...
Next
    ...
    wks1.Range("R3").Value = d
    wks1.Range("R3").Font.Bold = True
    wks1.Range("D3").MergeArea.Cells(1, 1).Value = Planinformation
    wks1.Range("D3").MergeArea.Cells(1, 1).Font.Size = 22
...
Sub createbankdetail(directory As String, filename As String, Planinformation As String, d As String)
    ...
    Dim R As Integer, C As Integer, header As Integer
    R = wks1.UsedRange.Rows.count
    C = wks1.UsedRange.Columns.count
    header = 7                                                   'set header
    
    wks1.Range("A" & header + 1 & ":H" & R).Value = TWS2.Range("A" & header + 1 & ":H" & R).Value
    wks1.Range("j" & header + 1 & ":j" & R).Value = TWS2.Range("j" & header + 1 & ":j" & R).Value
    wks1.Range("l" & header + 1 & ":o" & R).Value = TWS2.Range("l" & header + 1 & ":o" & R).Value
    wks1.Range("A1:V" & header).Value = TWS2.Range("A1:V" & header).Value
    
    
    
    Set ra1 = TWS2.Range("F1")
    Set ra2 = TWS2.Range("F1")
    
    Dim i As Integer
    For i = 1 To 10
    
    Set ra1 = TWS2.Range("F:F").Find("total", ra1)
    
    TWS2.Rows(ra1.Row).Copy wks1.Rows(ra1.Row)
    
    If ra1.Row < ra2.Row Then Exit For
    Set ra2 = ra1
    ...
    wks1.Range("O3").Font.Bold = True
    wks1.Range("D3").MergeArea.Cells(1, 1).Value = Planinformation
    wks1.Range("D3").MergeArea.Cells(1, 1).Font.Size = 22
    
    wkb1.SaveAs directory & "\" & filename & " " & TWS2.Name & " " & Planinformation & " " & d & ".xls", FileFormat:=56
    wkb1.Close False
