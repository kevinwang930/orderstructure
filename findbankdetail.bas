Attribute VB_Name = "findbankdetail"
Sub findbankdetail()
...
'   Set TWS3 = ThisWorkbook.Worksheets("bank detail collect report")   'target worksheet
    ...
    Set BP = wks1.UsedRange.Find(middlename)
    ...
    Else
        ...
        TWS2.Range("B" & i + 7).Value = BP.Value
        TWS2.Range("C" & i + 7).Value = wks1.Range("b" & BP.Row).Value                  'find and set bank information in bank detail
        TWS2.Range("d" & i + 7).Value = wks1.Range("c" & BP.Row).Value
        TWS2.Range("e" & i + 7).Value = wks1.Range("d" & BP.Row).Value
        TWS2.Range("f" & i + 7).Value = wks1.Range("e" & BP.Row).Value
    End If
